...
class UsersTableSeeder extends Seeder
{
    public function run(): void
    {
        $users = [
            ...
            ['name' => 'Noah Jackson', 'email' => 'noah.jackson@gmail.com', 'phone' => 'noah.jackson@gmail.com', 'role' => 'user', 'password' => Hash::make('password')],
        ];

        DB::table('users')->insert($users);

        // Tambah poin otomatis berdasarkan total cost di repairs
        foreach ($users as $u) {
            if ($u['role'] === 'user') {
                $repairs = Repair::where('customer_name', $u['name'])->get();
                $totalCost = $repairs->sum('cost');
                $points = floor($totalCost / 100000); // contoh: 1 poin per 100 ribu rupiah

                User::where('email', $u['email'])->update(['points' => $points]);
            }
        }
    }
}