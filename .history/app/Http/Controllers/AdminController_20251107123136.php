...
class AdminController extends Controller
{
    ...
    public function index()
    {
        $data = [
            ...
            'repairs' => Repair::with(['user', 'payment'])->paginate(10),
            'payments' => Payment::with(['repair.user'])->paginate(10),
            'rewards' => LoyaltyReward::with('user')->paginate(10),
            'spareparts' => Sparepart::paginate(10),
            // Spareparts summary counts (use full table counts, not current page)
            'spareparts_total' => Sparepart::count(),
            'spareparts_in_stock' => Sparepart::where('stock', '>', 0)->count(),
            'spareparts_low_stock' => Sparepart::where('stock', '>', 0)->where('stock', '<=', 5)->count(),
            'spareparts_out_of_stock' => Sparepart::where('stock', '<=', 0)->count(),
        ];
    ...
    public function getSection($section)
    {
        ...
        switch ($section) {
            ...
            case 'spareparts':
                ...
                // Add overall summary counts for spareparts so the UI shows DB totals
                $data['spareparts_total'] = Sparepart::count();
                $data['spareparts_in_stock'] = Sparepart::where('stock', '>', 0)->count();
                $data['spareparts_low_stock'] = Sparepart::where('stock', '>', 0)->where('stock', '<=', 5)->count();
                $data['spareparts_out_of_stock'] = Sparepart::where('stock', '<=', 0)->count();
    ...
    public function search($section)
    {
        ...
        switch ($section) {
            ...
            case 'repairs':
                // Search by tracking_id, customer_name, phone_brand, phone_model, imei, complaint or related user name
                $data['repairs'] = Repair::with(['user', 'payment'])
                                    ->where('tracking_id', 'like', "%{$query}%")
            ...
            case 'payments':
                $data['payments'] = Payment::with(['repair.user'])
                                    ->where('id', 'like', "%{$query}%")
            ...
            case 'loyalty':
                $data['rewards'] = LoyaltyReward::with('user')
                                    ->where('id', 'like', "%{$query}%")
            ...
            case 'spareparts':
                $data['spareparts'] = Sparepart::where('name', 'like', "%{$query}%")
                ...
                // Always include overall spareparts summary counts (whole DB) so the cards remain accurate
                $data['spareparts_total'] = Sparepart::count();
                $data['spareparts_in_stock'] = Sparepart::where('stock', '>', 0)->count();
                $data['spareparts_low_stock'] = Sparepart::where('stock', '>', 0)->where('stock', '<=', 5)->count();
                $data['spareparts_out_of_stock'] = Sparepart::where('stock', '<=', 0)->count();
