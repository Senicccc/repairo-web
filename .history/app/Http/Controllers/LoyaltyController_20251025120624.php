        ...
        <div class="overflow-x-auto max-h-[540px] overflow-y-auto">
            <table id="repairsTable" class="min-w-full text-sm text-gray-800">
                ...
                <tbody>
                    ...
                    <tr class="text-center hover:bg-gray-50 transition border-b" data-repair-id="{{ $repair->id }}"
                        ...
                        <td class="border px-3 py-2">
                            ...
                            @elseif ($repair->status == 'finished' && (!$repair->payment || $repair->payment->status ==
                            'unpaid'))
...
<dialog id="addRepairModal" class="modal">
    <form method="POST" action="{{ route('repairs.store') }}" class="p-6 bg-white rounded shadow-md w-[480px] mx-auto">
        ...
        <textarea name="complaint" placeholder="Damage Description" required
            class="w-full border p-2 mb-3 rounded"></textarea>
        <input type="number" step="0.01" name="cost" placeholder="Estimated Cost (optional)"
    ...
    <form method="POST" action="{{ route('payments.store') }}" class="p-6 bg-white rounded shadow-md w-[400px] mx-auto">
        ...
        <div class="flex justify-end space-x-2">
            <button type="button" onclick="document.getElementById('addPaymentModal').close()"
                class="px-3 py-1 bg-gray-300 rounded hover:bg-gray-400">Cancel</button>
            <button type="submit"
                class="px-3 py-1 bg-emerald-600 text-white rounded hover:bg-emerald-700">Submit</button>
        </div>
    </form>
</dialog>
...
<dialog id="redeemModal" class="modal">
    <div class="p-6 bg-white rounded shadow-md w-[400px] mx-auto">
        ...
        <div class="flex justify-end gap-2">
            <button onclick="document.getElementById('redeemModal').close()"
                class="px-3 py-1 bg-gray-300 rounded hover:bg-gray-400">Cancel</button>
            <button onclick="checkRedeemCode()"
                class="px-3 py-1 bg-yellow-500 text-white rounded hover:bg-yellow-600">Check</button>
            <button id="confirmClaimBtn" onclick="confirmClaim()"
                class="px-3 py-1 bg-green-600 text-white rounded hover:bg-green-700 hidden">Confirm Claim</button>
        </div>
    </div>
</dialog>
...
function filterTable() {
    ...
    document.querySelectorAll('#repairsTable tbody tr').forEach(row => {
        row.style.display = [...row.children].some(td => td.innerText.toLowerCase().includes(filter)) ? '' :
            'none';
    });
}
...
document.querySelectorAll('#repairsTable tbody tr').forEach(row => {
    row.addEventListener('click', () => {
        document.querySelectorAll('#repairsTable tbody tr').forEach(r => r.classList.remove(
        'selected'));
...
function checkRedeemCode() {
    ...
    fetch("{{ route('loyalty.check') }}", {
        ...
        .then(data => {
            ...
            if (data.valid) {
                resultDiv.innerHTML =
                    `<div class="p-2 bg-green-100 text-green-700 rounded">Claim: ${data.claim}</div>`;
