...
class AdminController extends Controller
{
    ...
    // Update Sparepart
 // Update Sparepart - PERBAIKAN: Sesuaikan dengan enum database
public function updateSparepart(Request $request, $id)
{
    try {
        $sparepart = Sparepart::findOrFail($id);

        // PERBAIKAN: Gunakan enum values yang sesuai dengan database
        $validated = $request->validate([
            'name' => 'required|string|max:255',
            'brand' => 'required|string|max:255',
            'category' => 'required|string|in:Original,OEM,Aftermarket,Replica', // SESUAIKAN
            'compatible_models' => 'nullable|string',
            'price' => 'required|numeric|min:0',
            'stock' => 'required|integer|min:0',
        ]);

        // Handle compatible_models as JSON
        if ($request->has('compatible_models') && !empty($request->compatible_models)) {
            $compatibleModels = array_map('trim', explode(',', $request->compatible_models));
            $validated['compatible_models'] = json_encode($compatibleModels);
        } else {
            $validated['compatible_models'] = null;
        }
...
// Create Sparepart - PERBAIKAN JUGA
