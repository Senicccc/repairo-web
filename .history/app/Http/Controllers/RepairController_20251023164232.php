...
class RepairController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($user->role === 'technician') {
            // kalau pakai kolom technician_id
            if (Schema::hasColumn('repairs', 'technician_id')) {
                $repairs = Repair::where('technician_id', $user->id)->with(['user', 'payment'])->get();
            } else {
                // fallback kalau masih pakai kolom 'technician' (string)
                $repairs = Repair::where('technician', $user->name)->with(['user', 'payment'])->get();
            }
        } else {
            // admin, cashier, user bisa lihat semua data
    ...
    public function technicianDashboard()
{
    ...
    // Semua job milik teknisi (in_progress + finished)
    ...
    // Semua job pending bisa di-claim
    $availableJobs = Repair::where('status', 'pending')
                           ->with(['user', 'payment'])
                           ->get();

    return view('staff.technician_dashboard', compact('currentJobs', 'availableJobs', 'otherJobs'));
}
