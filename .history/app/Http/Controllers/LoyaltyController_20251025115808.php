...
class LoyaltyController extends Controller
{
    ...
    public function claimReward(Request $request)
    {
        ...
        // Determine reward type & value
        if ($request->reward_type === 'discount') {
            $rewardType = 'discount';
            $rewardValue = (int) $request->discount;
        } else {
            $rewardType = 'gift';
            $rewardValue = $request->gift; // gift1 atau gift2
        }
    ...
    // Redeem code at cashier for discount or gift
    ...
    public function check(Request $request)
    {
        ...
        if($reward){
            return response()->json([
                'valid' => true,
                'claim' => $reward->reward_type === 'discount' 
                            ? 'Discount Rp' . number_format($reward->reward_value)
                            : $reward->reward_value,
                'reward_id' => $reward->id
            ]);
        }
    ...
    public function confirmClaim(Request $request)
    {
        ...
        if(!$reward || $reward->status !== 'claimed'){
            return response()->json(['success' => false, 'message' => 'Reward not valid or already claimed']);
        }
