...
public function addSparepart(Request $request, $id)
{
    ...
    $data = $request->validate([
        ...
        'name' => 'required|string|max:255', // Field name wajib
    ...
    // Siapkan data untuk disimpan - PASTIKAN SEMUA KOLOM ADA
    $sparepartData = [
        'repair_id' => $repair->id,
        'sparepart_id' => $data['sparepart_id'],
        'name' => $data['name'], // INI YANG PERLU DITAMBAHKAN
        'quantity' => $data['quantity'],
        'price' => $data['price'],
        'source' => $data['source'],
        'category' => $data['category'] ?? null, // Juga tambahkan category
    ];

    Log::info('Creating repair_sparepart with data:', $sparepartData);

    // Create the repair sparepart record
    ...
    // Update repair's sparepart field for quick reference
