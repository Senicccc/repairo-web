...
class PaymentController extends Controller
{
    ...
    public function store(Request $request)
    {
        $data = $request->validate([
            ...
            'payment_method' => 'required|in:cash,transfer,ewallet', // UBAH: payment_method (bukan method)
            'status' => 'required|in:unpaid,paid', // UBAH: required
        ]);

        $payment = Payment::create([
            'repair_id' => $data['repair_id'],
            'amount' => $data['amount'],
            'payment_method' => $data['payment_method'], // UBAH
            'status' => $data['status'],
        ]);

        $repair = Repair::find($data['repair_id']);
        if ($repair && $payment->status === 'paid') {
            $repair->update(['status' => 'paid', 'cost' => $payment->amount]); // UBAH: status jadi 'paid'
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($payment->status === 'paid') {
            ...
            if ($repair) {
                $repair->update(['status' => 'paid']); // UBAH: status jadi 'paid'
