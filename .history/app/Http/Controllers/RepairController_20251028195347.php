...
public function update(Request $request, $id)
{
    ...
    // kalau role lain (misal cashier) biarin handle yang versi lama
    $data = $request->validate([
    ...
    public function addSparepart(Request $request, $id)
{
    $data = $request->validate([
        ...
        'source' => 'required|in:from_stock,customer_supplied',
    ]);

    $repair = Repair::findOrFail($id);

    $repair->spareparts()->create($data);

    if ($data['source'] === 'from_stock' && $data['sparepart_id']) {
        $sparepart = \App\Models\Sparepart::find($data['sparepart_id']);
