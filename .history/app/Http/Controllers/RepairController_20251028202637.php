...
{
    // Tampilkan semua repairs (admin/cashier)
    ...
    // Tambah sparepart ke perbaikan
public function addSparepart(Request $request, $id)
{
    // allowed sources must match your repair_spareparts.source enum
    ...
    // jika in_store dan sparepart_id diberikan -> kurangi stok
    if ($data['source'] === 'in_store' && $data['sparepart_id']) {
        $sp = \App\Models\Sparepart::find($data['sparepart_id']);
        if ($sp) {
            $sp->decrement('stock', $data['quantity']);
        }
    }

    // update repair.sparepart field legacy supaya tetap ada ringkasan (optional)
    // gabungkan nama spareparts ke string supaya view lama tetep menampilkan sesuatu
