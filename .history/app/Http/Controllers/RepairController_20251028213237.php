...
public function addSparepart(Request $request, $id)
{
    ...
    // Hapus field name dan category dari data yang akan disimpan
    $sparepartData = [
        'repair_id' => $repair->id,
        ...
        'price' => $data['price'],
        'source' => $data['source'],
        // 'name' dan 'category' dihapus karena tidak ada di tabel
    ];
    ...
    // Update repair's sparepart field dengan mengambil nama dari tabel spareparts
    $sparepartsList = RepairSparepart::where('repair_id', $repair->id)
        ->join('spareparts', 'repair_spareparts.sparepart_id', '=', 'spareparts.id')
        ->pluck('spareparts.name')
        ->toArray();
