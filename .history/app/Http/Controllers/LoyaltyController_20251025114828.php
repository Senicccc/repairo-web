...
class LoyaltyController extends Controller
{
    ...
    // Confirm claim (AJAX) oleh kasir
    public function confirmClaim(Request $request)
    {
        $reward = LoyaltyReward::find($request->reward_id);

        if(!$reward || $reward->status !== 'claimed') {
            return response()->json(['success' => false, 'message' => 'Code invalid or already used']);
        }

        $reward->status = 'used';
        $reward->save();

        return response()->json(['success' => true, 'message' => 'Reward successfully claimed']);
    }
