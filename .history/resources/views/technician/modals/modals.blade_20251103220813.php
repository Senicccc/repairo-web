...
function initializeEventListeners() {
    ...
    // Add in-store sparepart to list - FIXED: Langsung tambah ke pending list
    if (addSpBtn) {
        addSpBtn.addEventListener('click', async () => {
            ...
            const newSparepart = {
                id: null, // akan diisi setelah save ke database
            ...
            try {
                console.log('Sending sparepart data:', newSparepart);
                ...
                if (result.success) {
                    // Tambahkan ke pending list dengan ID dari database
    ...
    // Add external sparepart to list - FIXED: Langsung tambah ke pending list
    if (addExtSpBtn) {
        addExtSpBtn.addEventListener('click', async () => {
            ...
            try {
                console.log('Sending external sparepart data:', newSparepart);
...
async function loadSparepartsForRepair(repairId) {
    try {
        const response = await fetch(`/api/repairs/${repairId}/spareparts`, {
            headers: {
                'X-Requested-With': 'XMLHttpRequest',
                'Accept': 'application/json'
            }
...
async function handleFormSubmission() {
    ...
    try {
        ...
        // Update sparepart field di repair
        try {
