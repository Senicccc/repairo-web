...
function initializeEventListeners() {
    ...
    if (addSpBtn) {
        addSpBtn.addEventListener('click', async () => {
            ...
            // Kirim ke backend (PASTIKAN URL BENAR)
            const it = {
            ...
            try {
                ...
                if (!sparepartResponse.ok) {
                    ...
                    try {
                        ...
                        // Fallback: try to get plain text (HTML or other)
                ...
                alert('Network error saving sparepart');
            }
            // Reset selection
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (!Array.isArray(existingSpareparts)) {
                console.error('API returned non-array:', existingSpareparts);
            }
            ...
            if (pendingSpareparts.length === 0) {
                console.warn('No spareparts found for repair', id);
            }
        } else {
            const errText = await response.text();
            console.error('Failed to fetch spareparts:', response.status, errText);
        }
    } catch (error) {
...
// 🔥 FIXED: handleFormSubmission dengan spareparts yang benar
async function handleFormSubmission() {
    ...
    try {
        // 🔥 STEP 1: SIMPAN SPAREPARTS YANG BARU DULU
        const newSpareparts = pendingSpareparts.filter(sp => !sp.id);
        
        if (newSpareparts.length > 0) {
            console.log(`Saving ${newSpareparts.length} new spareparts...`);
            
            for (let it of newSpareparts) {
                const sparepartData = new URLSearchParams();
                sparepartData.append('name', it.name);
                sparepartData.append('sparepart_id', it.sparepart_id || '');
                sparepartData.append('quantity', it.quantity.toString());
                ...
                if (!sparepartResponse.ok) {
                    const errorResult = await sparepartResponse.json();
                    console.warn(`Failed to save sparepart "${it.name}":`, errorResult.message);
                } else {
                    console.log(`Sparepart "${it.name}" saved successfully`);
                }
            }
        }

        // 🔥 STEP 2: UPDATE REPAIR DATA
        const formData = new FormData(form);
        
    const repairResponse = await fetch(`/repairs/${repairId}/update-job`, {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'X-Requested-With': 'XMLHttpRequest'
        ...
        // 🔥 STEP 3: UPDATE SPAREPART FIELD DI REPAIR
        try {
