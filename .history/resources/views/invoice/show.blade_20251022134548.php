...
<div class="max-w-md mx-auto bg-white p-4 border rounded text-sm font-mono" id="invoiceArea">
    ...
    <div class="border-b py-2 mb-2">
        ...
        @if($repair->payment)
            ...
            <p><strong>Date:</strong> {{ $repair->payment->payment_date ? $repair->payment->payment_date->format('d M Y H:i') : '-' }}</p>
        @else
...
function printInvoice() {
    ...
    printWindow.document.write('<style>body{font-family:monospace;text-align:left;padding:10px;font-size:12px;} h2{text-align:center;} .border-t,.border-b{border-top:1px dashed #000;border-bottom:1px dashed #000;}</style>');
