                ...
                <tr class="text-center" data-repair-id="{{ $repair->id }}" data-cost="{{ $repair->cost ?? 0 }}">
                    ...
                    <td class="border px-3 py-2">
                        <span class="px-2 py-1 rounded text-sm
                            {{ $repair->status == 'finished' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700' }}">
...
<dialog id="addPaymentModal" class="modal">
    <form method="POST" action="{{ route('payments.store') }}" class="p-6 bg-white rounded shadow-md w-[400px] mx-auto">
        ...
        <h3 class="text-lg font-semibold mb-4">Pembayaran Servis</h3>
            ...
            <input type="number" name="amount" id="payment_amount" placeholder="Total Bayar" required class="w-full border p-2 mb-3 rounded">
...
<script>
    function openPaymentModal(id) {
        var tr = document.querySelector('tr[data-repair-id="'+id+'"]');
        var amount = tr ? tr.dataset.cost : 0;
