            ...
            <table id="repairsTable" class="min-w-full text-sm text-gray-800">
                ...
                <tbody>
                    ...
                    <tr class="text-center hover:bg-gray-50 transition border-b"
                        ...
                        <td class="border px-3 py-2">{{ $repair->cost ?? 0 }}</td>
                        <td class="border px-3 py-2">
...
<dialog id="redeemModal" class="modal">
    <div class="p-6 bg-white rounded shadow-md w-[400px] mx-auto text-center">
        ...
        <input type="text" id="redeem_code_modal" placeholder="Enter loyalty code" class="w-full border p-2 mb-3 rounded uppercase text-center">
        ...
        <div class="flex justify-center gap-2" id="redeemButtons">
            ...
            <button type="button" onclick="checkRedeemCode()" id="checkBtn" class="px-3 py-1 bg-yellow-500 text-white rounded hover:bg-yellow-600">Check</button>
        </div>
    </div>
</dialog>
...
function checkRedeemCode() {
    ...
    .then(data => {
        ...
        if (data.valid) {
            ...
            resultDiv.innerHTML = `<div class='p-2 bg-green-100 text-green-700 rounded'>Reward Found: ${data.claim}</div>`;
            buttonsDiv.innerHTML = `
                <button type="button" onclick="document.getElementById('redeemModal').close()" class="px-3 py-1 bg-gray-300 rounded hover:bg-gray-400">Cancel</button>
                <button type="button" onclick="claimReward()" class="px-3 py-1 bg-green-600 text-white rounded hover:bg-green-700">Claim</button>
            `;
        } else {
...
function claimReward() {
    ...
    .then(data => {
        ...
        if (data.success) {
            resultDiv.innerHTML = `<div class='p-2 bg-green-200 text-green-900 rounded font-semibold'>Code claimed successfully!</div>`;
            buttonsDiv.innerHTML = `
                <button type="button" onclick="document.getElementById('redeemModal').close()" class="px-4 py-1 bg-blue-600 text-white rounded hover:bg-blue-700">OK</button>
            `;
        } else {
