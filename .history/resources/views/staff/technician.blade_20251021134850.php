...
<div class="max-w-6xl mx-auto p-6">
    ...
    {{-- Current Job --}}
    <div class="mb-6">
        <h3 class="text-xl font-semibold mb-2">Your Current Job</h3>
        @if ($currentJob)
            <div class="bg-white p-4 rounded shadow">
                <p><strong>Tracking:</strong> {{ $currentJob->tracking_id }}</p>
                <p><strong>Customer:</strong> {{ $currentJob->user->name ?? '-' }}</p>
                <p><strong>Phone:</strong> {{ $currentJob->phone ?? ($currentJob->user->phone ?? '-') }}</p>
                <p><strong>Complaint:</strong> {{ $currentJob->complaint }}</p>
                <p><strong>Status:</strong> {{ ucfirst($currentJob->status) }}</p>
                <div class="mt-3">
                    <button onclick="openJobModal({{ $currentJob->id }})" class="bg-green-600 text-white px-3 py-1 rounded">Update & Finish</button>
                </div>
            </div>
        @else
            <div class="bg-gray-50 p-4 rounded">You have no active job. Pick one from available jobs.</div>
        @endif
    </div>
