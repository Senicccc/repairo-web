    ...
    try {
        ...
        if (pendingSpareparts.length > 0) {
            ...
            for (let i = 0; i < pendingSpareparts.length; i++) {
                ...
                // Skip jika sudah ada ID (berarti sudah ada di database)
                if (it.id) {
                    console.log(`⏭️ Sparepart ${i + 1} already exists in database, skipping`);
                    successCount++;
                    continue;
                }
            ...
            console.log(`📊 RESULTS: ${successCount}/${pendingSpareparts.length} spareparts processed successfully`);
