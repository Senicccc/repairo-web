    ...
    <div class="bg-white shadow-md rounded-lg overflow-hidden">
        <table class="min-w-full border border-gray-200">
            ...
            <tbody>
                ...
                <tr class="text-center" data-id="{{ $repair->id }}">
                    <td class="px-4 py-2 border">{{ $repair->tracking_id }}</td>
                    <td class="px-4 py-2 border">{{ $repair->user ? $repair->user->name : 'Guest' }}</td>
                    <td class="px-4 py-2 border">{{ $repair->device_type }} - {{ $repair->brand }} {{ $repair->model }}</td>
...
<dialog id="addPaymentModal" class="modal">
    <form method="POST" action="{{ route('payments.store') }}" class="p-6 bg-white rounded shadow-md w-[400px] mx-auto">
        ...
        <input type="number" name="amount" placeholder="Total Amount" required class="w-full border p-2 mb-3 rounded">
        ...
        <div class="flex justify-end space-x-2">
            ...
            <button type="submit" class="px-3 py-1 bg-emerald-600 text-white rounded">Confirm Payment</button>
        </div>
    </form>
</dialog>
