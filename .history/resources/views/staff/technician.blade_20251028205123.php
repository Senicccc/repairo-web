    ...
    <form method="POST" id="jobForm" class="p-6 bg-white rounded shadow-md w-[720px] mx-auto">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <div id="spListArea" class="max-h-36 overflow-auto border rounded p-2 mb-2"></div>

                <label class="block text-sm font-medium mb-1">Total Cost</label>
                <input type="number" id="totalCost" class="w-full border p-2 rounded bg-gray-100" readonly value="0">
            </div>
        </div>
...
function updateTotalCost() {
    let sum = pendingSpareparts.reduce((acc, sp) => acc + (sp.price * sp.quantity), 0);
...
saveAllBtn.addEventListener('click', async () => {
    ...
    formData.set('cost', totalCost.value); // total cost dari semua sparepart

    form.action = `/repairs/${repairId}`;
    try { await fetch(form.action, { method: 'POST', headers:{ 'X-CSRF-TOKEN': '{{ csrf_token() }}' }, body: formData }); }
