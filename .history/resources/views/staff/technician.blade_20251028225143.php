...
async function openJobModal(id) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Convert existing spareparts to appropriate format (INCLUDE ID)
            existingSpareparts.forEach(sp => {
                pendingSpareparts.push({
                    id: sp.id, // IMPORTANT for delete
...
document.getElementById('jobForm').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (pendingSpareparts.length > 0) {
            ...
            for (let i = 0; i < pendingSpareparts.length; i++) {
                ...
                try {
                    ...
                    sparepartData.append('name', it.name || 'Unknown Sparepart'); // Ensure name is always provided
        ...
        // Wait briefly before reloading to ensure all processes complete
        setTimeout(() => {
            location.reload();
        }, 1000);
