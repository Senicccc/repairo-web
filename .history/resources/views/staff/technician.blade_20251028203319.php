...
addSpBtn.addEventListener('click', () => {
    if (!selectedSparepart) return alert('Choose a sparepart first')
...
function renderPendingList() {
    ...
    pendingSpareparts.forEach((it, i) => {
        const el = document.createElement('div')
        el.className = 'flex justify-between items-center gap-2 p-2 border-b text-sm'
        el.innerHTML = `<div><div class='font-medium'>${it.name}</div><div class='text-xs text-gray-600'>Qty: ${it.quantity} · Rp ${Number(it.price).toLocaleString()} · ${it.source}</div></div><button class='px-2 py-1 bg-red-500 text-white rounded' onclick='removePending(${i})'>Remove</button>`
        spListArea.appendChild(el)
    })
}
function removePending(i){ pendingSpareparts.splice(i,1); renderPendingList() }
...
saveAllBtn.addEventListener('click', async () => {
    ...
    try {
        await fetch(url, { method: 'POST', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' }, body: formData })
