...
<!-- Redeem Loyalty Code Modal (baru) -->
<dialog id="redeemModal" class="modal">
    <form method="POST" action="{{ route('loyalty.redeem') }}" class="p-6 bg-white rounded shadow-md w-[400px] mx-auto">
        ...
        <input type="text" name="redeem_code" id="redeem_code_modal" placeholder="Enter loyalty code" required 
               class="w-full border p-2 mb-3 rounded" value="{{ old('redeem_code') }}">
...
<!-- Redeem Code Modal (untuk kasir) -->
<dialog id="redeemModal" class="modal">
    <form method="POST" action="{{ route('loyalty.check') }}" class="p-6 bg-white rounded shadow-md w-[400px] mx-auto">
        @csrf
        <h3 class="text-lg font-semibold mb-4">Redeem Loyalty Code</h3>
...
function checkRedeemCode() {
    ...
    .then(data => {
        ...
        if(data.valid) {
            resultDiv.innerHTML = `<div class="p-2 bg-green-100 text-green-700 rounded">
                Code valid! Claim: ${data.claim}
            </div>`;
        } else {
            resultDiv.innerHTML = `<div class="p-2 bg-red-100 text-red-700 rounded">
                Code is not valid
            </div>`;
        }
    })
...
function checkRedeemCode() {
    ...
    .then(data => {
        const resultDiv = document.getElementById('redeemResult');
        if(data.valid) {
            resultDiv.innerHTML = `<div class="p-2 bg-green-100 text-green-700 rounded">${data.claim}</div>`;
