...
// Form submission handler - YANG SUDAH DIPERBAIKI
document.getElementById('jobForm').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        console.log('=== STEP 1: UPDATING REPAIR ===');
        ...
        if (pendingSpareparts.length > 0) {
            ...
            let sparepartSuccessCount = 0;
            
            for (const it of pendingSpareparts) {
                console.log('Processing sparepart:', it);
                ...
                sparepartData.append('quantity', it.quantity);
                sparepartData.append('price', it.price);
                sparepartData.append('source', it.source);
                sparepartData.append('category', it.category || '');

                console.log('Sending sparepart data to server:', Object.fromEntries(sparepartData));
                ...
                try {
                    ...
                    console.log('Sparepart API response:', sparepartResult);

                    if (sparepartResponse.ok) {
                        sparepartSuccessCount++;
                        console.log(`Sparepart ${sparepartSuccessCount} added successfully`);
                    } else {
                        console.error('Sparepart API error:', sparepartResult);
                        throw new Error(sparepartResult.message || 'Failed to add sparepart');
                    }
                } catch(sparepartErr) { 
                    console.error('Network error adding sparepart:', sparepartErr);
                    // Continue with other spareparts even if one fails
                }
            }
            
            console.log(`Successfully added ${sparepartSuccessCount} out of ${pendingSpareparts.length} spareparts`);
        } else {
            console.log('No spareparts to add');
        }
