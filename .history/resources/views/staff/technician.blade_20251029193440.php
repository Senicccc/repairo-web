            ...
            <div class="space-y-4">
                ...
                <div class="bg-white p-4 rounded shadow"
                    ...
                    <p><strong>Status:</strong> 
                        <span class="{{ \App\Models\Repair::getStatusColor($cj->status) }} px-2 py-1 rounded text-xs font-medium">
                            {{ \App\Models\Repair::getStatuses()[$cj->status] ?? ucfirst($cj->status) }}
                        </span>
                    </p>
                    ...
                    <p><strong>Cost:</strong> {{ $cj->cost ? 'Rp' . number_format($cj->cost, 0, ',', '.') : '-' }}</p>
                    <div class="mt-3">
            ...
            <table class="min-w-full border border-gray-200 text-sm">
                ...
                <tbody>
                    ...
                    <tr class="text-center">
                        ...
                        <td class="border px-3 py-2">
                            <span class="{{ \App\Models\Repair::getStatusColor($r->status) }} px-2 py-1 rounded text-xs font-medium">
                                {{ \App\Models\Repair::getStatuses()[$r->status] ?? ucfirst($r->status) }}
                            </span>
                        </td>
                        ...
                        <td class="border px-3 py-2">{{ $r->cost ? 'Rp' . number_format($r->cost, 0, ',', '.') : '-' }}</td>
...
async function openJobModal(id) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Convert existing spareparts to appropriate format
    ...
    try {
        ...
        if (pendingSpareparts.length > 0) {
            ...
            for (let i = 0; i < pendingSpareparts.length; i++) {
                ...
                // Skip if already has ID (already exists in database)
                if (it.id) {
                    successCount++;
                    continue;
                }
                ...
                try {
                    ...
                    if (sparepartResponse.ok && sparepartResult.success) {
                        ...
                        // Save ID for future remove
                        if (sparepartResult.sparepart_id) {
                            pendingSpareparts[i].id = sparepartResult.sparepart_id;
                        }
                    } else {
                    ...
                    console.error('Network error for sparepart:', sparepartErr);
                    errorMessages.push(`Sparepart "${it.name}": Network error - please try again`);
                }
            }
            
            if (errorMessages.length > 0) {
                alert(`Successfully processed ${successCount} spareparts, but ${errorMessages.length} failed:\n\n${errorMessages.join('\n')}`);
            } else {
                console.log(`All ${successCount} spareparts processed successfully`);
            }
        }
