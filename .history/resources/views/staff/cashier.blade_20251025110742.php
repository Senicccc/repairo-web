...
<dialog id="redeemModal" class="modal">
    <form method="POST" action="{{ route('loyalty.redeem') }}" class="p-6 bg-white rounded shadow-md w-[400px] mx-auto">
        ...
        <input type="text" name="redeem_code" id="redeem_code_modal" placeholder="Enter loyalty code" required class="w-full border p-2 mb-3 rounded">
        <div class="mb-3">
            <span class="text-sm font-medium">Reward Preview:</span>
            <div id="rewardPreviewModal" class="text-gray-700 font-semibold mt-1">-</div>
        </div>
        <div class="flex justify-end space-x-2">
...
<script>
    ...
    const redeemInputModal = document.getElementById('redeem_code_modal');
    const rewardPreviewModal = document.getElementById('rewardPreviewModal');
    redeemInputModal.addEventListener('input', () => {
        const code = redeemInputModal.value.trim().toLowerCase();
        if(code.startsWith('discount')) rewardPreviewModal.innerText = 'Discount 10%';
        else if(code.startsWith('gift')) rewardPreviewModal.innerText = 'Free Accessory';
        else rewardPreviewModal.innerText = '-';
    });
</script>
@endsection
