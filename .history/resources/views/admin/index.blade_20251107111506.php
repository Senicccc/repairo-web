...
<script>
// =============================================
// GLOBAL ADMIN FUNCTIONS - COMPLETELY FIXED
// =============================================
...
// =============================================
// PAGINATION & SEARCH FUNCTIONS - FIXED
// =============================================
...
function initializeSectionFunctions(section) {
    ...
    // Initialize search
    initializeSearchForSection(section);
...
// Search Initialization - FIXED
function initializeSearchForSection(section) {
    const searchInputId = getSearchInputId(section);
    ...
    if (searchInput) {
        ...
        console.log('🔍 Search initialized for:', section);
    }
}

function getSearchInputId(section) {
    const searchIds = {
        'users': 'userSearch',
...
function performServerSearch(section, query) {
    ...
    fetch(`/admin/search/${section}?q=${encodeURIComponent(query)}`)
        ...
        .catch(error => {
            ...
            // Fallback ke section normal
            showSection(section);
        });
}

// Pagination Initialization - FIXED
function initializePaginationForSection(section) {
    // Handle pagination links
    document.addEventListener('click', function(e) {
        if (e.target.matches('.pagination a, .pagination-link')) {
            e.preventDefault();
            const url = e.target.href;
            loadPage(section, url);
        }
    });
}

// Load page dengan pagination - FIXED
