...
function initializeSearch(searchInput, tableBody, section) {
    if (searchInput && tableBody) {
        searchInput.addEventListener('input', function(e) {
            ...
            // Update count jika ada element count
            updateSectionCount(section, visibleCount);
            console.log('🔍 Found', visibleCount, section, 'matching:', query);
        });
        console.log('🔍', section, 'search initialized');
    }
}

// Update count untuk setiap section
function updateSectionCount(section, count) {
    const countElement = document.querySelector(`#${section} .text-sm.text-gray-600`);
    if (countElement) {
        countElement.textContent = `Total: ${count} ${section}`;
    }
}
