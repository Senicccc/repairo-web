    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div class="mt-4 flex items-center justify-between">
            <div class="text-sm text-gray-600">
                Showing {{ $rewards->firstItem() ?? 0 }} to {{ $rewards->lastItem() ?? 0 }} of {{ number_format($rewards->total()) }} rewards
            </div>
            <div class="flex space-x-2">
                ...
                @else
                    <button onclick="loadPage('loyalty', '{{ $rewards->previousPageUrl() }}')" class="px-3 py-1 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition duration-200">
                        Previous
                    </button>
                @endif

                @if ($rewards->hasMorePages())
                    <button onclick="loadPage('loyalty', '{{ $rewards->nextPageUrl() }}')" class="px-3 py-1 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition duration-200">
                        Next
                    </button>
                @else
                    <span class="px-3 py-1 border border-gray-300 rounded-md text-sm font-medium text-gray-400 bg-gray-100 cursor-not-allowed">
...
{{-- Loyalty Edit Modal --}}
<div id="editLoyaltyModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 items-center justify-center p-4 hidden">
