        ...
        <div class="bg-white rounded-lg w-1/3 p-6 relative">
            ...
            <form id="editRepairForm">
                ...
                <div class="mb-4">
                    ...
                    <select id="editStatus" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="editTechnician" class="mt-1 block w-full border-gray-300 rounded-md">
                </div>

                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700">Cost</label>
                    <input type="number" id="editCost" class="mt-1 block w-full border-gray-300 rounded-md">
                </div>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        editForm.addEventListener('submit', async (e) => {
            ...
            if(res.ok){
                // update row langsung
                const row = repairsTable.querySelector(`tr[data-id="${id}"]`);
                if(row){
                    row.querySelector('td:nth-child(4) span').textContent = data.status.replace('_',' ');
                    row.querySelector('td:nth-child(5)').textContent = data.technician || '-';
                    row.querySelector('td:nth-child(6)').textContent = data.cost ? 'Rp ' + new Intl.NumberFormat('id-ID').format(data.cost) : '-';
                }
                editModal.classList.add('hidden');
                alert(resp.message);
            } else {
                alert('Failed to update repair: ' + (resp.message || 'Unknown error'));
            }
        });
    });
    </script>
