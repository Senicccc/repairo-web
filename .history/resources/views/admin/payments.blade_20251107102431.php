<section id="payments" class="tab-section p-6">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <!-- PAGINATION UNTUK REPAIRS -->
    <div class="mt-4 flex items-center justify-between">
        <div class="text-sm text-gray-600">
            Showing {{ $repairs->firstItem() ?? 0 }} to {{ $repairs->lastItem() ?? 0 }} of {{ number_format($repairs->total()) }} repairs
        </div>
        <div class="flex space-x-2">
            ...
            @else
                <button onclick="loadPage('repairs', '{{ $repairs->previousPageUrl() }}')" class="px-3 py-1 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition duration-200">
                    Previous
                </button>
            @endif

            @if ($repairs->hasMorePages())
                <button onclick="loadPage('repairs', '{{ $repairs->nextPageUrl() }}')" class="px-3 py-1 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition duration-200">
                    Next
                </button>
            @else
                <span class="px-3 py-1 border border-gray-300 rounded-md text-sm font-medium text-gray-400 bg-gray-100 cursor-not-allowed">
