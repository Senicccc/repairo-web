...
<div class="max-w-7xl mx-auto p-6">
    ...
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-blue-800">Cashier Dashboard</h2>
    ...
    <div class="mb-4">
        <input type="text" id="searchBar" 
               placeholder="Search repairs by customer, device, or tracking ID..." 
    ...
    <div class="bg-white shadow-lg rounded-lg overflow-hidden border border-gray-200">
        <div class="overflow-x-auto max-h-[540px] overflow-y-auto">
            <table id="repairsTable" class="min-w-full text-sm text-gray-800">
                ...
                <tbody>
                    @forelse ($repairs->sortByDesc('created_at') as $repair)
                    <tr class="text-center hover:bg-gray-50 transition border-b"
                        data-repair-id="{{ $repair->id }}">
                        <td class="border px-3 py-2 font-mono text-sm text-blue-700">
                            <div>{{ $repair->tracking_id ?? 'N/A' }}</div>
                        ...
                        <td class="border px-3 py-2 text-left max-w-xs">
                            <div class="line-clamp-2">{{ Str::limit($repair->complaint, 80) }}</div>
                        </td>
                    ...
                    <tr>
                        <td colspan="10" class="py-10 text-center text-gray-500">
                            ...
                            <p class="font-semibold">No repairs found</p>
                            <p class="text-sm mb-4">Get started by creating a new repair record</p>
                            <a href="{{ route('cashier.repairs.create') }}" 
